...
class BookmarkStorage(StorageABC):
    ...
    async def insert_bookmark(self, user_id: UUID, film_id: UUID, timestamp: datetime):
    ...
    async def get_bookmarks(self, user_id: UUID) -> list[ShortBookmark] | None:
        bookmarks: list | None = await self.manager().get(self.collection, {self.user_id_field: {"$eq": user_id}})
        if not bookmarks:
            return None
